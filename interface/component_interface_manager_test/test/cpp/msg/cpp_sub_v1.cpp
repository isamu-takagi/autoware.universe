...
#include "interface/sample_message/v1.hpp"
...
private:
  using SampleMessage = interface::sample_message::v1::Message;
  cimcpp::Subscription<SampleMessage>::SharedPtr sub_;
  void on_msg(const SampleMessage & msg);
};

TestNode::TestNode() : Node("cpp_sub_v1")
{
...
{
  RCLCPP_INFO(get_logger(), "%.2f %.2f", msg.velocity, msg.steering);
}
